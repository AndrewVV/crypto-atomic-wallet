...
import LitecoinTestLib from "../libs/LitecoinTestLib.js";
import {
    LTCTEST,
    BTCTESTAPIPROVIDER,
    LTCTESTAPIPROVIDER,
...
export default class AtomicSwapLtcTest extends LitecoinTestLib{
    
    async createOrderInDB(data){
        try{
            let url = "http://localhost:8600/create/order";
            data["status"] = CREATED_ORDER;
            data["addressBuyerToReceive"] = await this.generateAddAndPriv.generateAddress(LTCTEST);
    ...
    monitoringBuyer(id){
        let monitoring = setInterval(async() => {
            ...
            if(status == REPLIED_TO_ORDER && order.txHashBtc != ""){
                let url = `${LTCTESTAPIPROVIDER}get_tx/LTCTEST/${order.txHashBtc}`;
                let result = await this.httpService.getRequest(url).then(response=>response.json());
                let confirmations = result.data.confirmations
                if(confirmations > 0) {
    ...
    async replyToOrder(id, recipientPublicKey, buyAmount, addressBuyerToReceive){
        ...
        let addressSellerToReceive = await this.generateAddAndPriv.generateAddress(LTCTEST)
    ...
    async withdrawRawTransaction(data, isRefund) {
        ...
        const tx  = new bitcoinjs.bitcoin.TransactionBuilder(this.network) 
        ...
        let recipientAddress = await this.generateAddAndPriv.generateAddress(LTCTEST);
    ...
    async signTransaction(data, inputIndex = 0) {
        ...
        let recipientPrivateKey = await this.generateAddAndPriv.generatePrivKey(LTCTEST);
        let keyring = bitcoinjs.bitcoin.ECPair.fromWIF(recipientPrivateKey,this.network);
    ...
    async privKeyToPublicKey(){
        ...
        let keyPair = bitcoin.ECPair.fromWIF(privKey,this.network)
