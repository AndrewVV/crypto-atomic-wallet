...
export default class EdcLib {
    ...
    createAccount(nameAccount){
        return new Promise(async(resolve,reject)=>{
            try{
				...
				let result = await this.httpService.postRequest(EDC_REGISTER_URL, body)
	...
	formatFrontTxData(timeStamp, id, action, amount, moneyQuantity, hash, from, to, txFee, memo){
		let txData = {
            ...
            cryptoAmount: amount,
            moneyQuantity,
            copy: `https://explorer.blockchain.mn/block/${hash}`,
            explorer: `https://explorer.blockchain.mn/block/${hash}`,
            fromAddress: from,
            toAddress: to,
			txFee,
			memo,
