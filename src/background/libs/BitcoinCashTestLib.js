...
export default class BitcoinCashTestLibClass{
    ...
    getBalance(raw=true){
        return new Promise(async(resolve,reject)=>{
            try{
                ...
                let url = `${BCHTESTAPIPROVIDER}address/details/${address}`
    ...
    sendTransaction(to,amount,fee){
        return new Promise(async(resolve,reject)=>{
            try{
                ...
                let url = `${BCHTESTAPIPROVIDER}rawtransactions/sendRawTransaction/${rawTx}`
                let result = await this.httpService.getRequest(url).then(response=>response.json());
    ...
    listUnspent(address){
        return new Promise(async(resolve,reject)=>{
            try{
   	            this.validator.validateBtcAddress(address);
   	            let url = `${BCHTESTAPIPROVIDER}address/utxo/${address}`
    ...
    getTxHistory(){
        return new Promise(async(resolve,reject)=>{
            try{
                let address = await this.generateAddAndPriv.generateAddress(BCHTEST);
                let result = [];
                let url = `${BCHTESTAPIPROVIDER}address/transactions/${address}`
                ...
                if(allTx && allTx.txs.length > 0){
                    ...
                    for(let txKey in allTx){
                        let tx = allTx[txKey];
                            ...
                            let to = tx.vout[0].scriptPubKey.addresses[0];
                            let action;
                            if(to != from){
                                if(address == to){
                                    action = DEPOSIT;
	...
	formatFrontTxData(timeStamp, id, action, status, amount, moneyQuantity, hash, from, to, txFee){
		let txData = {
            ...
            moneyQuantity,
            copy: hash,
            explorer: `http://testnet.imaginary.cash/tx/${hash}`, // 'https://explorer.bitcoin.com/tbch/tx/'+hash,
            fromAddress: from,
            toAddress: to,
            txFee, 
		};
    ...
    async validateAddress(address){
        ...
        if(address){
            return validator.validate(address, BCH, TESTNET);
        }else return false
    }
    ...
    getFee(){
        return new Promise(async(resolve,reject)=>{
            try{
                let url = `${BCHTESTAPIPROVIDER}control/getNetworkInfo`;
                let result = await this.httpService.getRequest(url).then(response=>response.json())
                result = {
